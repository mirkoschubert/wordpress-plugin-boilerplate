...
final class Admin
{
  ...
  public function enqueue_scripts($hook)
  {
    ...
    if ($hook !== 'settings_page_wp-plugin-boilerplate') {
      return;
    }
    ...
    // Unsere React-App
    $js_file_path = $this->config->plugin_dir . 'admin-app/build/admin-app.js';
    $js_file_url = $this->config->plugin_url . 'admin-app/build/admin-app.js';

    if (!\file_exists($js_file_path)) {
      error_log("WPPluginBoilerplate: React Admin JS file not found: {$js_file_path}");
      return;
    }

    wp_enqueue_script(
      'wp-plugin-boilerplate-admin-app',
      $js_file_url,
    ...
    wp_set_script_translations('wp-plugin-boilerplate-admin-app', 'wp-plugin-boilerplate', $this->config->plugin_dir . 'languages');

    wp_localize_script('wp-plugin-boilerplate-admin-app', 'wpPluginBoilerplateConfig', [
      'apiUrl' => rest_url('wp-plugin-boilerplate/v1/'),
      'nonce' => wp_create_nonce('wp_rest'),
      'version' => $this->config->plugin_version ?? '1.0.0',
  ...
  public function render_page()
  {
    ...
    echo '<div id="wp-plugin-boilerplate-react-admin"></div>';
  }
