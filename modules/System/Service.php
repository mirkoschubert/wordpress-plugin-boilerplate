...
class Service extends ModuleService
{
  ...
  public function enqueue_badge_css()
  {
    ...
    wp_enqueue_style('wp-plugin-boilerplate-system-badge', $this->module->get_asset_url('css/system-badge.css'));
  }
  ...
  public function add_environment_badge(\WP_Admin_Bar $wp_admin_bar)
  {
    ...
    if ($this->is_option_enabled('search_visibility_warning') && get_option('blog_public') === '0') {
      $se_icon = ' <span class="wpbp-se-warning" title="'
        . esc_attr__('Search engines are discouraged from indexing this site', 'wp-plugin-boilerplate')
        . '">&#9888;</span>';
    }

    // neuen Titel aufbauen: alter Titel + Badge
    $new_title = $site_name->title
      . ' <span class="wpbp-environment-badge wpbp-env-' . esc_attr($env) . '">'
      . $se_icon
      . esc_html($label)
      . '</span>';
  ...
  public function display_system_summary()
  {
    ...
    echo '<p class="wpbp-system-summary" style="padding:0 12px 4px;margin:0;">'
      . implode(' &#9642; ', $parts)
      . ' &#9642; '
