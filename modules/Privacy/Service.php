...
class Service extends ModuleService
{
  ...
  public function log_last_login($user_login, $user)
  {
    if (\is_object($user) && \property_exists($user, 'ID')) {
      update_user_meta($user->ID, 'wp_plugin_boilerplate_last_login', \time());
    }
  }
  ...
  {
    $columns['wpbp_last_login'] = __('Last Login', 'wp-plugin-boilerplate');
    return $columns;
  }
  ...
  {
    if ($column_name !== 'wpbp_last_login') {
      return $output;
    }

    $last_login = get_user_meta($user_id, 'wp_plugin_boilerplate_last_login', true);

    if (empty($last_login)) {
      return __('Never', 'wp-plugin-boilerplate');
    }
