...
const ImageField = forwardRef<HTMLDivElement, ImageFieldProps>((
  ...
  const openMedia = useCallback(() => {
    ...
    const frame = window.wp.media({
      title: config.label || __('Select Image', 'wp-plugin-boilerplate'),
      multiple: isMulti,
      library: { type: 'image' },
      button: {
        text: isMulti
          ? __('Select Images', 'wp-plugin-boilerplate')
          : __('Select Image', 'wp-plugin-boilerplate'),
      },
    })
      ...
      <div className="image-field-header">
        ...
        <Button variant="secondary" onClick={openMedia} size="compact">
          {attachments.length > 0
            ? isMulti
              ? __('Add More', 'wp-plugin-boilerplate')
              : __('Replace', 'wp-plugin-boilerplate')
            : isMulti
              ? __('Select Images', 'wp-plugin-boilerplate')
              : __('Select Image', 'wp-plugin-boilerplate')}
        </Button>
          ...
          {attachments.map((att) => (
            <div key={att.id} className="image-field-item">
              ...
              <Button
                ...
                size="small"
                isDestructive
                label={__('Remove', 'wp-plugin-boilerplate')}
